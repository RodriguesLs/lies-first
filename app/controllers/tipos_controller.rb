class TiposController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @tipo.save
        format.html { redirect_to tipos_path, notice: I18n.t('messages.created') }
  ...
  def update
    respond_to do |format|
      if @tipo.update(tipo_params)
        format.html { redirect_to tipos_path, notice: I18n.t('messages.updated') }
  ...
  def destroy
    @tipo.destroy
    respond_to do |format|
      format.html { redirect_to tipos_path, notice: I18n.t('messages.destroyed') }
  ...
  private
    ...
    def tipo_params
      params.require(:tipo).permit(:desc)
    end
end
